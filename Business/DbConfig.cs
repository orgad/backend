namespace dotnet_wms_ef.Models
{
    public class DbConfig
    {
        ...
        {
            get{return string.Format(formatString,server,port,"wms_inbound",user,password);}
        }

        public static string OutboundDb
        {
            get{return string.Format(formatString,server,port,"wms_outbound",user,password);}
        }

        public static string InventoryDb
        {
            get {return string.Format(formatString,server,port,"wms_inventory",user,password);}
        }

        public static string WarehouseDb
        {
            get {return string.Format(formatString,server,port,"wms_warehouse",user,password);} 
        }

        public static string CustomerDb
        {
            get {return string.Format(formatString,server,port,"wms_customer",user,password);} 
        }
